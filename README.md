...
Проект намеренно не зависит от Qt, Electron или webview. Графический интерфейс построен на Dear ImGui, GLFW и OpenGL; CMake загружает и собирает Dear ImGui/GLFW как статические библиотеки. Разбор firmware интегрирован через локальные C++-обертки вокруг включенных исходников UEFITool, а извлечение HII/IFR выполняется собственным нативным парсером без Universal IFR Extractor.
...
Dear ImGui и GLFW загружаются через CMake `FetchContent`. Universal IFR Extractor не входит в дерево исходников и не используется при сборке.
